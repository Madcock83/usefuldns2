...
public class MCreatorMadsworld extends Elementsusefuldns.ModElement {
	...
	public MCreatorMadsworld(Elementsusefuldns instance) {
		super(instance, 58);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("madsworld"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("usefuldns:madsworld")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("usefuldns:madsworld"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("usefuldns:madsworld"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ModTrigger().setRegistryName("madsworld"));
	}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == MCreatorDecablo.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == MCreatorDecablo.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == MCreatorDecablo.block.getDefaultState().getBlock())) {
								break label56;
							}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos, flag ? MCreatorDecablo.block.getDefaultState().getBlock()
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, MCreatorDecablo.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getTo() == type) {
			{
				...
				MCreatorMadsworldPlayerEntersDimension.executeProcedure($_dependencies);
			}
		}
	}
		...
		public ChunkProviderModded(World world, BiomeProvider provider) {
			super(world, provider, new NetherGenSettings() {
				...
				public BlockState getDefaultFluid() {
					return MCreatorLiquidRF.block.getDefaultState();
				}
			});
